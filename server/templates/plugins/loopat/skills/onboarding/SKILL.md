---
description: 引导新用户认识 loopat —— 通过 8 个轻量阶段介绍核心概念（loop/context/vault/mount/memory/mcp）、教用户做一次真实配置、最后引导去开第一个真 loop。当用户首次进入 onboarding loop、说"开始引导"、或访问 /loopat:onboarding 时调用。
---
...
- **Workspace MCPs** —— admin 在 `knowledge/.loopat/claude/claude.json` 里配的，团队共享配置
- **Personal MCPs** —— 你自己加到 `personal/<user>/.loopat/claude/claude.json`，only you 能看到
...
- **看到 server 但都是 needs auth** → 引导用户授权：「点旁边的 "needs auth" 按钮，浏览器会跳到 OAuth 授权页，授权完跳回来 token 就存 vault 了。**注意**：授权完成不会自动对当前 loop 生效，需要点 popover 底部的 "Reload session" 按钮（保留对话历史）。」
- **看到 connected server** → 「太好了，已经有 server 可用。你可以让我调用对应工具，比如 `mcp__<server>__<tool>`。」
- **完全没看到 server** → 「你的 workspace 还没配任何 MCP server。如果是 admin，可以加到 `/loopat/context/knowledge/.loopat/claude/claude.json`（这是 ro 的，需要用户在 Context tab 里编辑提交）。如果是普通 user，可以加 personal MCP 到 `/loopat/context/personal/.loopat/claude/claude.json`。」
