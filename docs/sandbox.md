...
| /personal/`<user>`/.loopat/vaults/`<active>` | 1 per (user, vault)，沙箱内通过 `/loopat/context/vault` symlink 暴露 | per-loop 选 active；非 active vault 在沙箱里物理可见但 doctrine 告诫不要碰 |
...
- **`/loopat/context/vault`** —— sandbox 里的 symlink，指向 `personal/<user>/.loopat/vaults/<active>/`，所以本质上是 personal 子路径共享
...
| `/loopat/context/personal/` | `LOOPAT_HOME/personal/<user>/`（symlink） | rw | 用户私有；含 `memory/`、`.loopat/vaults/<name>/` 等 |
| `/loopat/context/vault` | symlink → `/loopat/context/personal/.loopat/vaults/<active>/` | rw | active vault 的便捷入口；非 active vault 也物理可见但 doctrine 告诫不读 |
...
每个 loop 在 `meta.config.vault` 上选一个 vault（默认 `"default"`）。沙箱里有一个 symlink `/loopat/context/vault` → `/loopat/context/personal/.loopat/vaults/<active>/`，AI 通过它访问凭据。
...
整个 `personal/` 是 wholesale bind 进沙箱的，所以**所有 vault 在 `/loopat/context/personal/.loopat/vaults/<name>/` 都物理可见**（v6 早期版本曾用 tmpfs 遮其他 vault，导致 git 误判"文件被删"——具体讨论见 git/overlay 兼容性的 trade-off）。doctrine 明确告诫 AI 只通过 `/loopat/context/vault` 入口访问 active vault，不要直接读其他 vault。

选了 vault=prod 时，沙箱内：

```
/loopat/context/vault → /loopat/context/personal/.loopat/vaults/prod/
                        ├── github-token (symlink to default/)
                        ├── provider-keys/anthropic (symlink)
                        └── provider-keys/openai
```

**历史 trade-off**（重要）：v6.x 早期用 per-file `--bind` 把 active vault 文件挂到沙箱 `personal/.loopat/vault/`（单数，沙箱内合成路径），catalog 用 tmpfs 遮。但 bwrap 在 bind 目标不存在时会**在 host 上创空 stub** 作为挂载点——沙箱退出后 stub 留在 host `personal/.loopat/vault/`，被 auto-commit 进 personal git。改成 symlink + wholesale bind 后这个泄漏消失。
