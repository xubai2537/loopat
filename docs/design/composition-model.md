...
### 例外：MCP 服务器走 SDK option（plugin 不再需要）

`.claude/` 里的内容分两类：

| 类别 | 例子 | 内容存在哪 | settings.json 起的作用 |
|---|---|---|---|
| **A. 自包含** | skills/ · agents/ · CLAUDE.md · hook 脚本 | 就在 `.claude/` 里 | SDK 读 settings 就够了 |
...
**类别 B 里 plugin 之前曾经是问题——现在不是了。** Plugin `enabledPlugins: {"foo@bar": true}` 只是指针，真正的 plugin 文件在 `~/.claude/plugins/cache/...`。SDK 要解析这个指针得读 `~/.claude/plugins/installed_plugins.json`。早期 loopat 的 sandbox 把 `$HOME` 整个隔离掉了，所以 SDK 解析不了——只能在 host 侧解析后通过 SDK `plugins:` option 把绝对路径直接喂进 sandbox。

**当前实现把 `~/.claude/plugins/` 整个 ro-bind 进 sandbox**，SDK 就能原生解析 `enabledPlugins` 了，不再需要 `plugins:` option 走 marketplace plugin。loopat 在 host 侧的活只剩：
1. `ensureLoopPluginsInstalled` — 跑 `claude plugin install`，把 enabled 但没装的 plugin 装上（sandbox 里 SDK 不会自己装）
2. `ensureExtraMarketplaces` — 跑 `claude plugin marketplace add/remove`，处理 URL drift

**仍然走 SDK option 的两个例外：**

| Option | loopat 怎么用 | 为什么 |
|---|---|---|
| `plugins: [{type:"local", path: BUILTIN_LOOPAT_PLUGIN_PATH}]` | 只传 loopat builtin 一项 | builtin plugin 在 `LOOPAT_INSTALL_DIR`，**不在** `~/.claude/plugins/`——SDK 自己找不到 |
| `mcpServers: {...}` | host 侧把 vault 凭据 merge 进 mcpServer 配置后传 | settings.json 写不下 runtime-resolved 凭据（`apiKey` 等） |

设计上的 tradeoff：wholesale bind 让 sandbox 看到 host 装过的**所有** plugin（虽然只有 enabled 的会激活），换来代码大幅简化。如果未来要更强的可见性隔离，可以改成 selective bind（只 bind enabled 的几个），但 install state 仍然是 host 全局共享的——这是 CC 自身的设计决定的。
...
- **builtin plugin（loopat 自带）** —— 通过 `plugins:` option 传 `LOOPAT_INSTALL_DIR/...` 路径（因为它不在 `~/.claude/plugins/` 里）；其他 marketplace plugin 走 settings.json 的 `enabledPlugins` 由 SDK 原生解析
- **mcpServers** —— 合并的 mcpServers 会被 vault 凭据注入后通过 SDK option 传，原因见 §2
...
| **loopat 里怎么启用** | 在某层 `.claude/skills/` 放进去——compose 把它符号链接进 user tier。在 NewLoopDialog 选对应 profile / 写到 team workspace / 放 personal 都行 | 同 skill：放到某层 `.claude/agents/` 就自动可用 | 在某层 `.claude/settings.json` 的 `mcpServers` 写——compose merge 后由 SDK option 注入（顺带把 vault 凭据并进去） | 在某层 `.claude/settings.json` 的 `enabledPlugins` 写 `true`——compose merge 后 `ensureLoopPluginsInstalled` 调 `claude plugin install`；bwrap 把 `~/.claude/plugins/` 整个 ro-bind 进 sandbox；SDK 在 sandbox 内原生解析 enabledPlugins | 在某层 `.claude/settings.json` 的 `hooks` 写——通过 settingSources `'user'` 生效 | 在某层 `.claude/mise.toml` 写——bwrap 启动前 `mise install + env --json`，把 PATH/env 注入 sandbox |
| **Sandbox 里落地路径** | `loops/<id>/.claude/skills/<name>/` (symlink → 源 host 路径) | `loops/<id>/.claude/agents/<name>.md` (symlink → 源 host 路径) | 不落文件——通过 SDK 进程内部传递 | `~/.claude/plugins/{installed_plugins.json,marketplaces/,cache/}` 整个 ro-bound；SDK 通过 settings.json 的 enabledPlugins + installed_plugins.json 解析 installPath | `loops/<id>/.claude/settings.json` 的 `hooks` 字段；脚本本身在源 tier 的 host 路径上（被知识 repo bind 或 personal bind 覆盖） | `loops/<id>/.claude/mise.toml` + bwrap `--setenv` 把 PATH 注入 |
